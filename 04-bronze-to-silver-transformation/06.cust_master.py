...
# setuo logging for audit purpose
...
cust_master_drop_dup = cust_master_drop_na2.dropDuplicates(["customer_id", "customer_payment_terms", "customer_created_date"])
print(f"before dropping dup: {cust_master_drop_na2.count()} | after dropping dup: {cust_master_drop_dup.count()}")
#cust_master_drop_dup.columns
...
write_to_silver(
    cust_master_drop_dup,
    silver_cust_master_table,
    "t.customer_id = s.customer_id AND t.customer_created_date = s.customer_created_date",
    [
        "customer_name",
        "customer_segment",
