...
# validation purpose: has a count of 16,966
#display(sales_order_df.count())
# order number is disintct
...
# merging with other tables and retrieving columns from this tables:

    

# pre-aggregating the payment_df
payment_by_invoice = (
    payment_df
    ...
    .agg(
        ...
        F.round(F.sum("payment_amount"), 2).alias("total_paid"),
        # method belonging to the most recent payment
        F.max(F.struct("payment_date", "payment_method")).alias("_last"),
    )
    .withColumn("payment_method", F.col("_last.payment_method"))
    .withColumn("part_paid_flag",
                F.when(F.col("payment_count") > 1, "Y").otherwise("N"))
    .drop("_last")
)


# buiilding the fact_sales_order_df
sales_order_selected_df = (
    sales_order_df.alias("so")
    .withColumn("order_sk", F.xxhash64(F.col("order_number").cast("string")))
    .join(invoice_df.alias("i"), F.col("so.order_number") == F.col("i.order_id"), "left")
    .join(F.broadcast(exchange_rates_df.alias("e")),
          ...
          "left")
    .join(dim_customer_df.alias("dc"), F.col("so.customer_id") == F.col("dc.customer_id"), "left")
    .join(dim_customer_account.alias("ac"), F.col("so.customer_id") == F.col("ac.account_id"), "left")
    .join(F.broadcast(channels_df.alias("c")), F.col("so.channel_code") == F.col("c.channel_code"), "left")
    .join(payment_by_invoice.alias("p"), F.col("i.invoice_number") == F.col("p.invoice_number"), "left")
    .select(
        F.col("order_sk"),
...
#display(sales_order_selected_df.count()) # 16,966 count
#display(sales_order_selected_df.groupBy("order_sk").agg(F.count("*").alias("count")).filter(F.col("count")>1))# no duplicates
#sales_order_selected_df.columns
